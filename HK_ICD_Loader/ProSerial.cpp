...
// ProSerial
void ProSerial::LoadIcdFile(CString strFilePath)
{
...
void ProSerial::ReadSheetData(int nCurSheetNo,vector<vector<string>>& vtSheetData)
{
	if(nCurSheetNo ==-1)
		return;
...
void ProSerial::GetTempData(vector<vector<string>>& vtTemp, CString csSheetNo)
{
	vtTemp.clear();
...
void ProSerial::Initailize(void)
{
	// 获取协议模板
	CString strPath = GetExePath();
	CString strTempFile;
	strTempFile.Format("%s\\Template\\Serial_DISP.xlsx",strPath);
...
void ProSerial::GetTemplate(vector<vector<string>>& vtTemp, CString csSheetNo)
{
...
TREEDATA& ProSerial::BuildProtocolTree(TREEDATA& ParentTree,int nProType,int nDlgId,int& nTreeId)
{
	m_nSysDriveId = -1;
	...
	for(int i = 1,end = vec.size(); i < end; ++ i)
	{
		...
		// 设置通道编号
		sTxt.Format("%d",m_nChannelNo);
		vtBlockData[4][5] = sTxt.GetBuffer();
...
BOOL ProSerial::BuilSubProTree(TREEDATA& subTreeData, int nProSheetNo,int nProType,int& nTreeId)
{
	if(nProSheetNo == -1)
		return FALSE;
...
CString ProSerial::GetExePath(void)  
{  
	CString strExePath;  
...
// 获得模板的头部和尾部数据
void ProSerial::GetTempHeadTail(vector<vector<string>>& vtTemp,vector<vector<string>>& vtTHead,
...
void ProSerial::InitialRowData(vector<vector<string>>& vtTemp,vector<string>& vtRowData,int nType)
{
...
BOOL ProSerial::BuildDBTree(TREEDATA& DBTree, __SYS_Drive& DInforItem, int nProType, int& nTreeId,int& nStatus)
{
	...
	for(int i = 0; i < nProNum; ++ i)
	{
		...
		if(xml.FindElem(XML_Attr))
		{
			...
			// 协议类型
			sTxt = xml.GetAttrib(XML_ProType_CN);
			vtProData2[1][0] = sTxt.GetBuffer();
			// 协议名称
			sTxt = xml.GetAttrib(XML_Name_CN);
			vtProData2[0][3] = sTxt.GetBuffer();
			// 描述
			CString sBProNo = xml.GetAttrib(XML_Describe_CN);
			vtProData2[1][3] = sBProNo.GetBuffer();
			vtRowData1[0] = sBProNo.GetBuffer();
			TreeSubItem.sProNo = sBProNo;
			// 版本
			sTxt = xml.GetAttrib(XML_Ver_CN);
			vtProData2[0][5] = sTxt.GetBuffer();
			// 数据单位
			CString sDataUnit = xml.GetAttrib(XML_DataUnit_CN);
			vtProData2[2][2] = sDataUnit.GetBuffer();
			// 发送模式（传输类型）
			sTxt = xml.GetAttrib(XML_SendMode_CN);
			vtProData2[3][2] = sTxt.GetBuffer();
			// 接收或发送
			sTxt = xml.GetAttrib(XML_RecSend_CN);
			vtProData2[4][2] = sTxt.GetBuffer();
			// 校验方式（校验位）
			sTxt = xml.GetAttrib(XML_CheckMode_CN);
			vtProData2[5][2] = sTxt.GetBuffer();
			// 数据块ID
			sTxt = xml.GetAttrib(XML_BlockId_CN);
			vtProData2[6][2] = sTxt.GetBuffer();
			// 数据长度
			sTxt = xml.GetAttrib(XML_DataLen_CN);
			vtProData2[2][5] = sTxt.GetBuffer();
			// 刷新时间（传输周期）
			sTxt = xml.GetAttrib(XML_RefreshTime_CN);
			vtProData2[3][5] = sTxt.GetBuffer();
			// 通道编号
			sTxt = xml.GetAttrib(XML_ChannelNo_CN);
			...
			////接收协议包增加应答条件，条件值及应答协议
			////if(sProtocolRecSend == PROTOCOL_RecSend_Rec)
			//{
			//	//应答条件
			//	sTxt = xml.GetAttrib(XML_Reply_Condition);
			//	vtProData2[8][2] = sTxt.GetBuffer();
			//	//条件值
			//	sTxt = xml.GetAttrib(XML_Reply_Value);
			//	vtProData2[9][2] = sTxt.GetBuffer();
			//	//应答协议
			//	sTxt = xml.GetAttrib(XML_Reply_protocol);
...
void ProSerial::GetSubAttr(CMarkup& xml,TREEDATA& TreeSubItem,
	...
	vector<string>& vtRowTData,CString sTDataUnit, int& nTreeId, int nProType)
{
	...
	{
		// 协议名称（数据据块名称）
		sProName = xml.GetAttrib(XML_BlockName_CN);
		vtProData3[0][3] = sProName.GetBuffer();
		// 规范号（描述）
		sProNo = xml.GetAttrib(XML_Describe_CN);
		vtProData3[1][3] = sProNo.GetBuffer();
		//// 数据类型
		//vtProData3[2][2] = sItemType;
		// 数据单位
		sDataUnit = xml.GetAttrib(XML_DataUnit_CN);
		vtProData3[2][2] = sDataUnit.GetBuffer();
		// 数据长度
		sDataLen = xml.GetAttrib(XML_DataLen_CN);
		vtProData3[3][2] = sDataLen.GetBuffer();
	...
	else
	{
		...
		vtProData3[1][3] = "";
		// 数据单位
		vtProData3[2][2] = sTDataUnit.GetBuffer();
		// 数据长度
...
void ProSerial::AttachTailTemp(vector<vector<string>>& vtProData, vector<vector<string>>& vtTempTail)
{
...
void ProSerial::DeleteBLOCKFromDB(TREEDATA& TreeNode)
{
...
void ProSerial::SaveDataToDB(TREEDATA& TreeData, int nCurDrive)
{
	...
	for(int i = nRow,end = vec.size(); i < end /*&& false*/; ++ i)
	{
		...
		if(sBlockType == "ITEM" /*&& false*/)
		{
			...
			//协议类型
			CString sTxt = vtItem[1][0].c_str();
			xml.AddAttrib(XML_ProType_CN, sTxt);
			//协议名称（取树中的信号名称）
			xml.AddAttrib(XML_Name_CN,sProtocolName);
			//描述（协议号）
			sTxt = vtItem[1][3].c_str();
			xml.AddAttrib(XML_Describe_CN,sTxt);
			//版本
			sTxt = vtItem[0][5].c_str();
			xml.AddAttrib(XML_Ver_CN, sTxt);
			//数据单位
			CString sDataUnit = vtItem[2][2].c_str();
			xml.AddAttrib(XML_DataUnit_CN,sDataUnit);
			//传输类型
			sTxt = vtItem[3][2].c_str();
			xml.AddAttrib(XML_SendMode_CN, sTxt);
			//接收/发送--取主目录的属性
			xml.AddAttrib(XML_RecSend_CN,sProtocolRecSend);
			//校验方式
			sTxt = vtItem[5][2].c_str();
			xml.AddAttrib(XML_CheckMode_CN,sTxt);
			//数据块ID
			sTxt = vtItem[6][2].c_str();
			xml.AddAttrib(XML_BlockId_CN,sTxt);
			//数据长度
			sTxt = vtItem[2][5].c_str();
			xml.AddAttrib(XML_DataLen_CN,sTxt);
			int nDataLen = _tstoi(sTxt);
			//刷新时间（传输周期）
			sTxt = vtItem[3][5].c_str();
			sTxt.Trim("ms");// 去掉ms
			xml.AddAttrib(XML_RefreshTime_CN,sTxt);
			//通道编号
			sTxt = vtItem[4][5].c_str();
			xml.AddAttrib(XML_ChannelNo_CN,sTxt);
...
void ProSerial::GetDataAttr(CMarkup& hxml, TREEDATA& TreeData, int nIdx, CString sRecSend,
	CString sItemNo, CString sProtocoName, int nProtocolId)
{
	...
	//解析协议属性
	//数据块名称
	CString sTxt = vec[0][3].c_str();
	hxml.AddAttrib(XML_BlockName_CN,sTxt);
	//描述
	sTxt = vec[1][3].c_str();
	hxml.AddAttrib(XML_Describe_CN,sTxt);
	////版本
	//sTxt = vec[0][5].c_str();
	//hxml.AddAttrib(XML_Ver_CN,sTxt);
	//数据单位
	sTxt = vec[2][2].c_str();
	hxml.AddAttrib(XML_DataUnit_CN,sTxt);
	//数据长度
	sTxt = vec[3][2].c_str();
	hxml.AddAttrib(XML_DataLen_CN,sTxt);
...
void ProSerial::SetProgressFunc(CallFunc func,LPVOID lpParam)
{
	m_lpParam = lpParam;
	m_func = func;
}
