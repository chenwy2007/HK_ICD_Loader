...
// ProCAN
void ProCAN::LoadIcdFile(CString strFilePath)
{
...
void ProCAN::ReadSheetData(int nCurSheetNo,vector<vector<string>>& vtSheetData)
{
	if(nCurSheetNo ==-1)
		return;
...
void ProCAN::GetTempData(vector<vector<string>>& vtTemp, CString csSheetNo)
{
	vtTemp.clear();
...
void ProCAN::Initailize(void)
{
	// 获取协议模板
	CString strPath = GetExePath();
	CString strTempFile;
	strTempFile.Format("%s\\Template\\CAN_DISP.xlsx",strPath);
...
void ProCAN::GetTemplate(vector<vector<string>>& vtTemp, CString csSheetNo)
{
...
TREEDATA& ProCAN::BuildProtocolTree(TREEDATA& ParentTree,int nProType,int nDlgId,int& nTreeId)
{
	m_nSysDriveId = -1;
...
BOOL ProCAN::BuilSubProTree(TREEDATA& subTreeData, int nProSheetNo,int nProType,int& nTreeId)
{
	if(nProSheetNo == -1)
		return FALSE;
...
CString ProCAN::GetExePath(void)  
{  
	CString strExePath;  
...
// 获得模板的头部和尾部数据
void ProCAN::GetTempHeadTail(vector<vector<string>>& vtTemp,vector<vector<string>>& vtTHead,
...
void ProCAN::InitialRowData(vector<vector<string>>& vtTemp,vector<string>& vtRowData,int nType)
{
...
BOOL ProCAN::BuildDBTree(TREEDATA& DBTree, __SYS_Drive& DInforItem, int nProType, int& nTreeId,int& nStatus)
{
...
void ProCAN::GetSubAttr(CMarkup& xml,TREEDATA& TreeSubItem,
...
void ProCAN::AttachTailTemp(vector<vector<string>>& vtProData, vector<vector<string>>& vtTempTail)
{
...
void ProCAN::DeleteBLOCKFromDB(TREEDATA& TreeNode)
{
...
void ProCAN::SaveDataToDB(TREEDATA& TreeData, int nCurDrive)
{
...
void ProCAN::GetDataAttr(CMarkup& hxml, TREEDATA& TreeData, int nIdx, CString sRecSend,
...
// CAN协议子项存入共享内存数据库
void ProCAN::SaveProtocolItemConfig(void)
{
	int nItem_ConfigId;
	int nItem_SharedId;
...
void ProCAN::SetProgressFunc(CallFunc func,LPVOID lpParam)
{
	m_lpParam = lpParam;
	m_func = func;
}
