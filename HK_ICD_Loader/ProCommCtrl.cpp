...
// ProCommCtrl
void ProCommCtrl::LoadIcdFile(CString strFilePath)
{
...
void ProCommCtrl::ReadSheetData(int nCurSheetNo,vector<vector<string>>& vtSheetData)
{
	if(nCurSheetNo ==-1)
		return;
...
void ProCommCtrl::GetTempData(vector<vector<string>>& vtTemp, CString csSheetNo)
{
	vtTemp.clear();
...
void ProCommCtrl::Initailize(void)
{
	// 获取协议模板
...
void ProCommCtrl::GetTemplate(vector<vector<string>>& vtTemp, CString csSheetNo)
{
...
TREEDATA& ProCommCtrl::BuildProtocolTree(TREEDATA& ParentTree,int nProType,int nDlgId,int& nTreeId)
{
	m_nSysDriveId = -1;
	...
	for(int i = 1,end = vec.size(); i < end; ++ i)
	{
		...
		if(it_pro != m_protocoNameVec.end())
		{
			...
			m_excelFile.ReadInfo_Sub_Item(nProSheetNo+1,vtBlockData,1,-1,12);
		}
		string sRecSend = RowDataItem[5];
		int nRecSend = sRecSend == "显示" ? 1:2;
...
BOOL ProCommCtrl::BuilSubProTree(TREEDATA& subTreeData, int nProSheetNo,int nProType,int& nTreeId)
{
	if(nProSheetNo == -1)
		return FALSE;
	...
	vector<vector<string>>vec;
	m_excelFile.ReadInfo_Sub_Item(nProSheetNo+1,vec,1,-1,12);
	...
	for(int i = 0,end = vec.size(); i < end; ++ i)
	{
		...
		if(!bFindStartFlag)
		{
			if(RowDataItem[3] == "信号名称")
				bFindStartFlag = true;
		}
		else if(RowDataItem[3] == "结束符")
			break;
		else
		{
			// 非ITEM项不构建子节点
			if(RowDataItem[5] != "ITEM")
				continue;

			TreeDataItem.lID = nTreeId ++;
			TreeDataItem.nProType = nProType;
			TreeDataItem.sName = RowDataItem[1].c_str();
			TreeDataItem.sDataType = "ITEM";
...
CString ProCommCtrl::GetExePath(void)  
{  
	CString strExePath;  
...
// 获得模板的头部和尾部数据
void ProCommCtrl::GetTempHeadTail(vector<vector<string>>& vtTemp,vector<vector<string>>& vtTHead,
...
void ProCommCtrl::InitialRowData(vector<vector<string>>& vtTemp,vector<string>& vtRowData,int nType)
{
...
BOOL ProCommCtrl::BuildDBTree(TREEDATA& DBTree, __SYS_Drive& DInforItem, int nProType, int& nTreeId,int& nStatus)
{
...
void ProCommCtrl::GetSubAttr(CMarkup& xml,TREEDATA& TreeSubItem,
...
void ProCommCtrl::AttachTailTemp(vector<vector<string>>& vtProData, vector<vector<string>>& vtTempTail)
{
...
void ProCommCtrl::DeleteBLOCKFromDB(TREEDATA& TreeNode)
{
...
int ProCommCtrl::GetProType(CString& sProType)
{
	int nProType = -1;

	if(sProType == "1553B")
		nProType = 0;
	else if(sProType == "Serial")
		nProType = 1;
	else if(sProType == "Arinc429")
		nProType = 2;
	else if(sProType == "CAN")
		nProType = 3;
	else if(sProType == "mDAQ")
		nProType = 4;

	return nProType;
}

void ProCommCtrl::SaveDataToDB(TREEDATA& TreeData, int nCurDrive)
{
	...
	for(int i = nRow,end = vec.size(); i < end /*&& false*/; ++ i)
	{
		...
		//组号
		CString sGroupId = vtRowData[3].c_str();
		int nGroupId = _tstoi(sGroupId);
		//协议类型
		CString sProType = vtRowData[4].c_str();
		int nProType = GetProType(sProType);
		//接收/发送
		CString  sProtocolRecSend= vtRowData[5].c_str();

		CString strXml = "";
		BOOL bFindBlockProtocol = FALSE;

		if(sBlockType == "PAGE" /*&& false*/)
		{
			...
			xml.AddAttrib(XML_CTRL_BTYPE,sTxt);
			//控件行数
			sTxt = vtItem[3][2].c_str();
			xml.AddAttrib(XML_CTRL_ROWNUM,sTxt);
			//控件列数
			sTxt = vtItem[4][2].c_str();
			xml.AddAttrib(XML_CTRL_COLNUM,sTxt);

			bFindBlockProtocol = XProtocol_Funtion::GetProCommCtrlId(m_nDlgId,nProType,sProtocolName,sProtocolRecSend,m_nProtocolId);
			// 协议数据项都从1开始编号
			...
			for(int j = 0; j < nBRow; ++ j)
			{
				...
				if(bFindStart)
				{
					...
					xmlChild.AddAttrib(XML_PRO_NAME,sTxt);
					//编号
					CString sItemNo = vtSubItem[2].c_str();
					...
					xmlChild.AddAttrib(XML_RecSend,sTxt);
					//项类型
					CString sItemType = vtSubItem[5].c_str();
					...
					// ITEM 项不加入表达式及转义值属性
					//组号
					sTxt = vtSubItem[8].c_str();
					//sTxt.MakeLower();
					//sTxt.Trim();
					xmlChild.AddAttrib(XML_Item_Group,sTxt);
					//行号
					sTxt = vtSubItem[9].c_str();
					xmlChild.AddAttrib(XML_Item_RowNo,sTxt);
					//列号
					//sTxt = "";
					sTxt = vtSubItem[10].c_str();
					xmlChild.AddAttrib(XML_Item_ColNo,sTxt);
					//控件类型
					sTxt = vtSubItem[11].c_str();
					xmlChild.AddAttrib(XML_CTRL_DType,sTxt);
			...
			XProtocol_Funtion::SaveProCommCtrl(
				m_nDlgId,
				m_nProtocolId,
				sProtocolRecSend,
				sProtocolName,
...
void ProCommCtrl::GetDataAttr(CMarkup& hxml, TREEDATA& TreeData, int nIdx, CString sRecSend,
	CString sItemNo, CString sProtocoName, int nProtocolId)
{
	...
	//解析协议属性
	//表达式
	CString sTxt = vec[3][2].c_str();
	hxml.AddAttrib(XML_Item_Express,sTxt);
	//信号数
	sTxt = vec[4][2].c_str();
	hxml.AddAttrib(XML_Item_SignalNum,sTxt);
	...
	for(int j = 0; j < nBRow; ++ j)
	{
		...
		if(bFindStart)
		{
			...
			//项编号
			sTxt = vtSubItem[1].c_str();
			xmlSubChild.AddAttrib(XML_Item_Code,sTxt);
			//协议块名称
			sTxt = vtSubItem[2].c_str();
			xmlSubChild.AddAttrib(XML_PRO_NAME,sTxt);
			int nSubInforLength = int(_tstoi(sTxt)/8.0+1);
			//编号
			sTxt = vtSubItem[3].c_str();
			xmlSubChild.AddAttrib(XML_Item_NO,sTxt);
			//信号名称
			sTxt = vtSubItem[4].c_str();
			xmlSubChild.AddAttrib(XML_Name,sTxt);
			//接收/发送
			sTxt = vtSubItem[5].c_str();
			xmlSubChild.AddAttrib(XML_RecSend,sTxt);
			//项类型
			sTxt = vtSubItem[6].c_str();
			//sTxt.MakeLower();
			//sTxt.Trim();
			...
			//单位
			sTxt = vtSubItem[7].c_str();
			xmlSubChild.AddAttrib(XML_Item_Unit,sTxt);
			//范围
			sTxt = vtSubItem[8].c_str();
			xmlSubChild.AddAttrib(XML_Item_Range,sTxt);
...
void ProCommCtrl::SetProgressFunc(CallFunc func,LPVOID lpParam)
{
	m_lpParam = lpParam;
	m_func = func;
}
