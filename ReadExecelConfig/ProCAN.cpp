...
BOOL ProCAN::BuildDBTree(TREEDATA& DBTree, __SYS_Drive& DInforItem, int nProType, int& nTreeId,int& nStatus)
{
	...
	for(int i = 0; i < nProNum; ++ i)
	{
		...
		if(xml.FindElem(XML_Attr))
		{
			...
			// 邮箱标识
			sTxt = xml.GetAttrib(XML_MailBoxID);
			vtProData2[4][5] = sTxt.GetBuffer();
			// 标识符长度
			sTxt = xml.GetAttrib(XML_MaskLen);
			...
			// 帧类型
			sTxt = xml.GetAttrib(XML_Frame_Type);
			vtProData2[7][2] = sTxt.GetBuffer();
			// 帧格式
			sTxt = xml.GetAttrib(XML_Frame_Format);
...
void ProCAN::SaveDataToDB(TREEDATA& TreeData, int nCurDrive)
{
	...
	for(int i = nRow,end = vec.size(); i < end /*&& false*/; ++ i)
	{
		...
		if(sBlockType == "ITEM" /*&& false*/)
		{
			...
			xml.AddAttrib(XML_MailBoxID,sTxt);
			//标识符长度
			sTxt = vtItem[5][5].c_str();
			xml.AddAttrib(XML_MaskLen,sTxt);
			//速率
			...
			xml.AddAttrib(XML_Frame_Type,sTxt);
			// 帧格式
			sTxt = vtItem[8][2].c_str();
			xml.AddAttrib(XML_Frame_Format,sTxt);
			//共享内存值
