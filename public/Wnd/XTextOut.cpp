...
XTextOut::XTextOut(float fScale,__TextAttr_V02 &attr)
{
	m_fScale =fScale;
...
bool XTextOut::RefreshData()
{
	...
	for(int i =0,end =m_RefreshEventVec.size();i<end;i++)
	{
		...
		if(  g_XShared.ReadData_Int(m_RefreshEventVec[i].nSharedId,nValue))
		{
			//区间值，或者等于某一个值
...
void XTextOut::SetCurTextColor(COLORREF ref)
{
	m_ColorCurFont =ref;
}
void XTextOut::ReSetTextColor()
{
	m_ColorCurFont = m_ColorDefaultFont;
}

void XTextOut::DrawInfoEx(CDC * pDC ,CRect rt)
{
	if(m_sTitle.empty()) 
		return;
